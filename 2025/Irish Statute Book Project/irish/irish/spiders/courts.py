...
class CourtSpider(Spider):
    ...
    def __init__(self):
        ...
        # files & Records
        os.makedirs('judgments_downloads', exist_ok=True)
    ...
    def start_requests(self):
        for year in range(2002, 2026):
            # year= '2017'
            #for 100 records
            # url = f'https://www.courts.ie/search/judgments/" type:Judgment" AND "filter:alfresco_radio.title" AND "filter:alfresco_Court.High court" AND "filter:alfresco_Court.Court of Appeal" AND "filter:alfresco_fromdate.01 Jan 2001" AND "filter:alfresco_todate.01 Jan {year}"'
    ...
    def parse(self, response, **kwargs):
        ...
        self.write_logs(f"Processing Year: {year} | Total Judgments Found: {total_records}")
    ...
    def pagination(self, response):
        ...
        for tr in table_tr:
            ...
            if name in self.previous_pdfs:
                print(f"[SKIP] '{name}' has already been scraped. Skipping to the next record...")
    ...
    def download_pdf(self, response):
        ...
        output_dir = os.path.join('judgments_downloads', str(year))
    ...
    def get_previous_records(self):
        try:
            # Get all PDF file paths from 'judgments_downloads' directory
            files = glob.glob('judgments_downloads/*/*.pdf')
