...
class EhiSpiderSpider(Spider):
    ...
    def parse_quote(self, response, **kwargs):
        ...
        json_data = self.get_json(zip_code, group_id)
        ...
        yield Request(url=update_census_url, method="POST", dont_filter=True, body=json.dumps(json_data),
                      headers=self.census_headers, cookies=self.census_cookies, callback=self.parse_update_census,
                      meta=response.meta)
    ...
    def get_json(self, zip_code, group_id):
